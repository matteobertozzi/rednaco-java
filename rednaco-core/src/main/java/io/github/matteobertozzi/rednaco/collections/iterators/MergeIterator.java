...
public class MergeIterator<T> implements PeekIterator<T> {
  ...
  public MergeIterator(final Comparator<? super T> comparator, final Iterable<? extends Iterator<? extends T>> iterators) {
    this.queue = new PriorityQueue<>(2, (a, b) -> comparator.compare(a.peek(), b.peek()));
    for (final Iterator<? extends T> iterator: iterators) {
      if (iterator.hasNext()) {
        this.queue.add(SimplePeekIterator.from(iterator));
      }
    }
  }
