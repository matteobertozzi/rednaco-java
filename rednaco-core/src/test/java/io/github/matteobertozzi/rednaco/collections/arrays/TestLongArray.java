...
public class TestLongArray {
  ...
  public void testRandAdd() {
    final LongArray items = new LongArray(256);
    ...
    for (int i = 0; i < 10_000; ++i) {
      ...
      for (int j = 0; j <= i; ++j) {
        Assertions.assertEquals(checkRand.nextLong(), items.get(j));
      }

      // verify forEach() single item
      checkRand.setSeed(seed);
      items.forEach(v -> Assertions.assertEquals(checkRand.nextLong(), v));

      // verify forEach() batch
      checkRand.setSeed(seed);
      items.forEach((arr, arrOff, arrLen) -> {
        for (int j = 0; j < arrLen; ++j) {
          Assertions.assertEquals(checkRand.nextLong(), arr[arrOff + j]);
        }
      });
    }
  }
